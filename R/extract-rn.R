...
rn.prep.coverage= function(rn.raw, direction="wide", first.year, final.year) {
  ...
  dat$risk_group <- names(strata.labels$risk.groups)[match(dat$risk_group, strata.labels$risk.groups)]
...
#' @return A data frame.
#' @export
rn.prep.method.mix = function(rn.raw, direction="wide", first.year, final.year) {
  ...
  dat$risk_group <- names(strata.labels$risk.groups)[match(dat$risk_group, strata.labels$risk.groups)]
  dat$prep_method <- names(strata.labels$prep.methods)[match(dat$prep_method, strata.labels$prep.methods)]
  attr(dat, "label") <- NULL
